...
## 19. Tutor Update Feature

### 19.1 Product Rationale

Tutor Update is a distribution and engagement feature: it makes the student's tutor look data-driven in sessions at zero effort for the tutor, while creating an organic referral surface for SAT Tutor Pro. The flywheel: student shares a link → tutor sees the branded report → tutor recommends the app to other students.

Key design decisions:
- **Curated, not raw.** The report shows skill gaps, error patterns, and positive trends — never mood signals, frustration flags, raw session logs, or the parent dashboard view. It is a coaching tool, not surveillance.
- **Contact-first sharing.** SMS and WhatsApp are the primary paths because tutors typically communicate via phone. Email and copy-link are fallbacks.
- **One-tap weekly send.** After the first setup, the student can send a fresh update in a single tap. The CTA resets automatically each Monday.
- **Seven-day expiry, reused within a week.** A new link is only created when the existing one has expired, so the same URL works all week.

### 19.2 Attribution Block

Every report page includes a small attribution callout at the very top, before any student data. It reads:

> "This update was generated by SAT Tutor Pro, an AI-powered SAT practice app that tracks how [first name] gets things wrong — not just what they get wrong — to personalize every study session. Learn more at sattutor.pro"

Dual purpose: useful context for the tutor (explains what the data means) and an organic marketing surface. Styled as a muted background callout — not a banner ad.
...
One row per student (active, unexpired). Older/expired rows are retained for historical reference but never surfaced.

### 19.4 API Routes

| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/api/tutor-update/create` | Required | Create or reuse an unexpired link for the current student |
| GET | `/api/tutor-update/report/[token]` | None (public) | Validate token, increment view_count, return report payload |
| POST | `/api/tutor-update/send` | Required | Send via email (Resend) or return SMS/WhatsApp deep link |

**create** upsert logic: if an unexpired link exists for the student, update tutor info and reuse it. Otherwise insert a new row with a fresh token.

**report/[token]** returns 404 if not found, 410 if expired. Payload includes: student first name only (no last name, email, or parent info), predicted score + delta vs 4 weeks ago, RW/Math sub-scores, days practiced and questions answered this week, top 3 weak skills with accuracy/trend/pattern note, top 3 skills trending up with before/after accuracy, and the top `wrong_answer_insights` finding with recommendation.

**send** channel behavior:
- `email` — sends HTML email via Resend. Subject: `[Student name]'s SAT Tutor Update — week of [date]`
- `sms` — returns `deep_link: sms:[phone]?body=[message]`; client opens it
- `whatsapp` — returns `deep_link: https://wa.me/[e164]?text=[message]`; client opens in new tab

Phone normalization: strip non-digits; if 10 digits prepend `+1`; if 11 digits starting with 1 prepend `+`; otherwise prepend `+` and pass through.

### 19.5 Public Report Page

Route: `app/report/[token]/page.tsx`

- Server Component — works with zero client-side JS (accessible on any device/browser)
- Public route — added to middleware allowlist
- No app navigation chrome — standalone page with just the report
- Metadata: `noindex`, title `[First name]'s SAT Tutor Update`
- Business logic shared via `lib/tutor-report.ts` (used by both this page and the API route)

**Error states:**
- Token not found → "Report not found" with instructions to ask the student for a new link
- Token expired → "This report has expired" with instructions to ask the student for a new link

### 19.6 Report Sections (in order)

1. **Attribution block** — muted callout explaining SAT Tutor Pro; links to sattutor.pro
2. **Report header** — student first name, week of, predicted score + delta, RW/Math sub-scores; brand blue #1E3A5F background
3. **Practice this week** — days practiced (out of 7) and questions answered, as stat cards; emerald green
4. **Focus for your next session** — top 3 weak skills: name, accuracy %, trend arrow (↑↓→), one-line pattern note from `SKILL_PATTERN_NOTES` map
5. **Wrong answer intelligence** — top insight from `wrong_answer_insights.top_insights[0]`; amber left border (#F59E0B); shows finding + recommendation styled callout
6. **Making progress** — top 3 trending-up skills with before → after accuracy and a progress bar; emerald green
7. **Footer** — "Generated by SAT Tutor Pro · sattutor.pro"

Color rules: brand blue for header, emerald for progress/practice stats, amber left border for AI insight card. Yellow #EAB308 is NOT used here (reserved for Wrong Answer Insights inside the app).

### 19.7 Share UI — TutorUpdateButton

Component: `components/tutor-update-button.tsx`
Placement: `app/progress/page.tsx`, between the page heading and the stat cards

**First-time flow:**
- Button: "Send tutor update" with Send icon, brand blue
- Click opens a Dialog modal with: tutor name (optional), contact value (phone/email)
- Four method buttons: Message (primary blue), WhatsApp (green), Email (outline), Copy link (outline)
- On send: calls `/api/tutor-update/create` then `/api/tutor-update/send`; handles channel-specific behavior
- Saves `{ name, value, contactType }` to `localStorage['tutor_contact']`

**Returning flow (tutor saved in localStorage):**
- Button: "Send update to [Tutor Name]" with Send icon
- Single tap → calls create + send → shows success toast
- "Change tutor" link below opens the modal again
- "Sent X days ago" muted label when `last_sent_at` is within the current week

**Auto-refresh:** `last_sent_at` is stored in `sessionStorage['tutor_last_sent_at']` after sending. The sent label resets automatically on new week start (Monday 00:00 local time).

**localStorage key:** `tutor_contact` — stores `{ name: string, value: string, contactType: string }`

### 19.8 Middleware Update

`/report/:path*` added to the public routes allowlist in `middleware.ts`. Unauthenticated users can access report pages directly without being redirected to login.
